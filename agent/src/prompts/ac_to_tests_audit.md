...
category-spread count without asserting any new observable behavior. And flag every test that violates the positive-anchor-before-absence rule (see the required rules
below for the exact matcher list this covers across every runner) -- a deterministic gate WILL
reject the whole draft for exactly this, so catching it in your revision saves a full redraft lap:
add the anchor assertion yourself in `revised_test_suite` rather than merely reporting it.
