"""P15 -- exit. One LLM judgment node (a StageSpec, ADVERSARIAL/audit pattern reused for
...
Verification status: NOT exercised against a real sandbox, same caveat as every P8+ node cluster
this session.
"""
...
async def p15_finalize_node(state: dict[str, Any], config: RunnableConfig) -> dict[str, Any]:
    ...
    header = "# Changelog\n\nAuto-generated by ai-dev-workflow's P15 exit stage.\n\n"
    ...
    await git_ops.commit_paths(
        provider, thread_id, [MANIFEST_PATH, snapshot_path, CHANGELOG_PATH], "ai-dev-workflow: p15 finalize (manifest, changelog)"
    )
    return {}
