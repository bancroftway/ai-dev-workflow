...
flowchart TD
    ...
    stage4["STAGE 4: AC-TO-TESTS<br/>Write failing tests (TDD red) for ELIGIBLE (undelivered) ACs only; delete tests naming retired ACs; a data-touching AC grounds its assertions in the ER diagram when one exists; an AC named in bug_affected_ac_ids gets a RED test that reproduces the REPORTED failure, not just a generic assertion<br/>Same-turn Stop hook (sandbox-image/hooks/check-test-quality-stop.mjs): before either draft/audit turn ends, every test file is checked for fiat-failure stubs, absence-only assertions, and near-duplicate bodies by shelling out to gates/test_quality_checks.py — the SAME pure-Python implementation deterministic_verify uses below, not a second JS port of it — so these are fixed in-turn instead of costing a whole redraft lap to discover<br/>Agents: ac-to-tests-draft → ac-to-tests-audit (scaffold-only rebuild)<br/>Gate: write-scope (a UI stack's playwright.config.* must set screenshot: 'on', content-checked, not just requested) + ledger-integrity + retired-residue + completed-AC protection + AC-coverage checks<br/>Post-scaffold TDD-red gate: zero passing tests (first ticket) / eligible-AC tests red (later tickets)"]
    
    stage5["STAGE 5: MINIMAL CODE TO GREEN<br/>Implement least code to pass tests; delete code serving only retired ACs; reads any ER/architecture diagram before designing the data model/module structure, and any user_flow diagram before wiring navigation<br/>Agents: minimal-code-to-green-draft → minimal-code-to-green-audit<br/>Gate: 95% line+branch coverage CONTRACT REPLAY (full rebuild)"]
...
<!-- graph-source-sha256: b2e44af9368a06a9e67a8220a76c7f6d16c990c258f62d5a80a5417bafb8c5c0 -->
