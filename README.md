...
flowchart TD
    ...
    stage4["STAGE 4: AC-TO-TESTS<br/>Write failing tests (TDD red) for ELIGIBLE (undelivered) ACs only; delete tests naming retired ACs; a data-touching AC grounds its assertions in the ER diagram when one exists; an AC named in bug_affected_ac_ids gets a RED test that reproduces the REPORTED failure, not just a generic assertion<br/>Agents: ac-to-tests-draft → ac-to-tests-audit (scaffold-only rebuild)<br/>Gate: write-scope (a UI stack's playwright.config.* must set screenshot: 'on', content-checked, not just requested) + ledger-integrity + retired-residue + completed-AC protection + AC-coverage checks<br/>Post-scaffold TDD-red gate: zero passing tests (first ticket) / eligible-AC tests red (later tickets)"]
...
<!-- graph-source-sha256: 868e2fd2fd74e92cb2d8a6dd388937e3e63fd7518d4e284918b360b5f2864b63 -->
