...
flowchart TD
    ...
    intake["INTAKE &nbsp;·&nbsp; normalize the run and decide what carries over from previous runs<br/>1. mint a fresh run_id (used by the spec ledger, APPROVALS.md and metrics-report/exit snapshots)<br/>2. pop the registry's one-shot `resume` meta flag unconditionally (it must never leak into a later, unrelated run); it only actually applies below when this thread had no in-memory stage state at all before hydration ran — a genuine restart-resume, not an ordinary follow-up message<br/>3. first invoke for this thread: hydrate every stage's state back out of the repo (workflow_persistence.py)<br/>4. seed default state for every StageSpec that has none yet<br/>5. reset specification onward to not_started — tech-stack and raw-requirements stay approved across runs; AIDW_RESUME=1 or the applied resume flag skips this reset instead, so a restart resumes at the first unapproved stage<br/>6. split the newest HumanMessage into raw_requirements_text plus any image/document attachments; an empty scan result falls back to the hydrated raw-requirements doc, so a resume never drafts the spec from nothing<br/>7. a blank run with no requirements anywhere (the frontend's reload/reattach ping) ends here — zero LLM calls"]
...
<!-- graph-source-sha256: 3532e627e443d7c041219552444de2172532e7aa0a8e6baf1a1b058177941ff5 -->
