...
flowchart TD
    ...
    intake["INTAKE &nbsp;·&nbsp; normalize the run and decide what carries over from previous runs<br/>1. mint a fresh run_id (used by the spec ledger, APPROVALS.md and metrics-report/exit snapshots)<br/>2. pop the registry's one-shot `resume` meta flag unconditionally (it must never leak into a later, unrelated run) and split the newest HumanMessage into raw_requirements_text plus any image/document attachments — the flag only actually applies below when THIS run carries no fresh requirements text: a live-thread Resume click fires a blank runAgent (no message) exactly like a restart's reattach ping, while a genuine new-requirements submission always has text and so can never be misread as a resume<br/>3. first invoke for this thread: hydrate every stage's state back out of the repo (workflow_persistence.py)<br/>4. seed default state for every StageSpec that has none yet<br/>5. reset specification onward to not_started — tech-stack and raw-requirements stay approved across runs; AIDW_RESUME=1 or the applied resume flag skips this reset instead, so a resume picks up at the first unapproved stage (in-memory checkpoint or repo hydration, whichever this thread has)<br/>6. an empty requirements scan falls back to the hydrated raw-requirements doc, so a resume never drafts the spec from nothing<br/>7. a blank run with no requirements anywhere (the frontend's reload/reattach ping) ends here — zero LLM calls"]
...
<!-- graph-source-sha256: 0307090c05046a195e511d85663f706082f1a1fbca5b26dacfdeb7ff9a2eacd6 -->
